...
- 私聊可使用流式飞书卡片回复；群聊默认保持普通文本聊天，不把操作按钮塞进群里。
- 本地延迟日志：记录读消息、检索记忆、通话纪要、首 token、最终发送等阶段耗时。
- 隐私优先的记忆系统：按 profile 隔离，默认本地 JSON 存储，支持本地向量和可见性过滤；候选记忆先私聊 owner 确认。
...
- 普通群聊不会直接弹出记忆按钮；只有明显像偏好、安排、称呼、重要事实的内容，才会私聊 owner 发候选记忆确认卡。
