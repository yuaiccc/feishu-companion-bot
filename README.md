...
- 每日恋爱笔记：`love_note.py` 每天按 `LOVE_NOTE_RUN_AT` 读取已有飞书 Wiki/Docx 恋爱笔记正文，用 DeepSeek 生成一条嗑糖短评，并以局部评论挂到文档最后一段非空正文上，不向正文追加内容。通过 `state.json` 的 `last_love_note_date` 保证每天只写一次；预览用 `python main.py --daily-note-preview`，手动写入测试用 `python main.py --daily-note-test`。
...
默认关闭，打开 `LOVE_NOTE_ENABLED=true` 后，本地长连接进程会启动每日评论线程。它读取现有恋爱笔记正文，把“这也太甜了、磕到了”这类短评作为飞书局部评论挂在文档最后一段非空正文上，不覆盖原文，也不向正文追加噪声。
