...
这是一个飞书**群聊应用**机器人（不是私聊机器人），配置在飞书群聊中。它的人设不是第三方助手，而是"秋酿/三哥"（许君山，学生）本人不在时的临时代替，用来陪"舒舒/烨子"（舒烨）说话、回答问题，并在必要时汇报 GitHub 活动。
...
`main.py` 的 `_classify_tool_intent()` 返回 `status`、`github` 或 `none`。问"在干嘛/最近活动/最近怎么样"默认只走本地窗口状态；明确问 GitHub、提交、commit、代码、仓库、PR、issue 才推 GitHub 卡片。不要再把普通状态问题自动带到 GitHub。
