...
这是一个飞书**群聊应用**机器人（不是私聊机器人），配置在飞书群聊中。帮用户"三哥"（许君山，学生）自动汇报 GitHub 活动给女朋友"舒舒"（舒烨）。

**重要**：机器人是飞书群聊中的应用，群聊消息只有 @机器人 才回复，私聊全部回复。其他 Agent 维护时请注意这个区别。

**核心功能：**
1. 本地 WebSocket 长连接，实时接收飞书消息并回复
2. GitHub 活动监控，有新 commit 自动推送飞书卡片
3. 本地应用检测（AppleScript），告诉舒舒三哥在用什么软件
4. DeepSeek 生成通俗总结 + 情感化回复
5. GitHub Actions 云端轮询，电脑关机时也能 5 分钟内回复
